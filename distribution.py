...
if args.shards != None:
    ...
    if True: #if args.distribution == "uniform":
        partition = np.split(
...
if args.requests != None:
    if args.distribution == "reset":
        ...
        if args.distribution.split(":")[0] == "pareto":
            ...
            all_requests = set()
            while len(all_requests) < args.requests:
                all_requests.add(int(np.random.pareto(a, 1)[0]))
            all_requests = np.array(list(all_requests))
        if args.distribution.split(":")[0] == "inverse_pareto":
            a = (
                float(args.distribution.split(":")[1])
                if len(args.distribution.split(":")) > 1
                else 1.16
            ...
            while i < len(all_requests):
                all_requests[i] = datasetfile["nb_train"] - 1 - all_requests[i]
                i += 1
        else:

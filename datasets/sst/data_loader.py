...
train_data = np.load(os.path.join(pwd, 'sst2_train.npy'), allow_pickle=True)
test_data = np.load(os.path.join(pwd, 'sst2_test.npy'), allow_pickle=True)
...
def load(indices, category='train'):
    ...
    if category == 'train':
        print(type(X_train[indices]), type(y_train[indices]))

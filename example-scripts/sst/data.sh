...
for j in {0..15}; do
    echo $j
    r=$((${j}*${shards}/5))
    python aggregation.py --strategy uniform --container "${shards}" --shards "${shards}" --dataset datasets/sst/datasetfile --label "${r}"
    acc=$(python aggregation.py --strategy uniform --container "${shards}" --shards "${shards}" --dataset datasets/sst/datasetfile --label "${r}")
